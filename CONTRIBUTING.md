...
> **Ruff formatting is enforced in CI.** The `Check ruff format` step in `.github/workflows/test-linting.yml` runs `ruff format --check` over the Python files your PR changes under `litellm/`, excluding `litellm/enterprise/`.
>
> `ruff.toml` is the single source of truth for formatting, including `line-length = 120`. Avoid configuring an 88-column formatter; it will fight CI on every save.
>
> - **AI coding agents** (Claude Code, Copilot, Cursor, etc.): run `make format` before committing.
...
make format                    # Apply Ruff code formatting
make format-check              # Check Ruff formatting (matches CI)
...
- **Ruff formatter** for consistent code formatting
- **Ruff linter** for linting and code quality
